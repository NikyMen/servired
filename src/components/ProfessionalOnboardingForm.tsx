...
export function ProfessionalOnboardingForm({ categories, localities, initial }: { categories: Category[]; localities: Locality[]; initial: Initial }) {
  ...
  return <div className="mx-auto max-w-3xl space-y-5">
    ...
    <div className="glass glass-solid space-y-5 rounded-2xl p-5 sm:p-6">
      ...
      {step === 4 && <section className="space-y-3"><h2 className="text-xl font-bold text-slate-900">Revisá antes de enviar</h2><dl className="grid gap-3 text-sm sm:grid-cols-2"><Summary label="Tipo" value={providerType} /><Summary label="Actividad" value={values.headline} /><Summary label="Rubros" value={String(categoryIds.length)} /><Summary label="Años en el oficio" value={values.yearsExperience || "0"} /><Summary label="Ubicación" value="Corrientes Capital, Corrientes, Argentina" /><Summary label="Cobro" value="Mercado Pago: se vincula después de la aprobación" /><Summary label="Localidad" value={selectedLocality ? `${selectedLocality.name}, ${selectedLocality.province}` : "—"} /><Summary label="Identidad" value="DNI frente, dorso y video listos" /></dl><label className="flex items-start gap-3 rounded-xl bg-white/70 p-3 text-sm text-slate-700"><input type="checkbox" checked={ofertasDependencia} onChange={(e) => setOfertasDependencia(e.target.checked)} className="mt-0.5 size-5 shrink-0" /><span>¿Además de las oportunidades vinculadas a tu oficio, te gustaría recibir ofertas laborales en relación de dependencia?</span></label><p className="rounded-xl bg-amber-50 p-3 text-sm text-amber-800">El perfil quedará pendiente hasta que administración revise la documentación.</p></section>}
