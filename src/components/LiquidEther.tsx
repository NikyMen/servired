...
/**
 * LiquidEther — simulación de fluido en WebGL, de React Bits (variante JS+CSS).
 *
 * Es código de terceros: se mantiene con la misma estructura que el original
 * para poder seguir sus actualizaciones, y sólo se le agregaron los tipos que
 * pide este repo. Los estilos de `.liquid-ether-container` viven en
 * globals.css, junto al resto de las clases de componentes, en vez de en un
 * .css al lado del archivo.
 *
 * Dos cosas para el que lo use:
 * - `colors` tiene que ser una constante estable (fuera del render). Si se le
 *   pasa un literal, cada render arma un array nuevo y el efecto reconstruye
 *   todo el contexto WebGL.
 * - No necesita eventos de puntero propios: escucha en window y compara contra
 *   su bounding rect, así que funciona con `pointer-events: none` encima.
 */
...
/** Lo que los efectos necesitan tocar del manager una vez creado. */
type WebGLHandle = {
  output: { simulation: { options: SimOptions; resize: () => void } } | null;
  autoDriver: {
    enabled: boolean;
    speed: number;
    resumeDelay: number;
    rampDurationMs: number;
    mouse: { autoIntensity: number; takeoverDuration: number } | null;
  } | null;
  start: () => void;
  pause: () => void;
  resize: () => void;
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const bgVec4 = new THREE.Vector4(0, 0, 0, 0); // siempre transparente
    ...
    class AutoDriver {
      ...
      constructor(
        ...
        this.speed = opts.speed; // unidades normalizadas por segundo
...
`;
    ...
    class ShaderPass {
      ...
      // Los argumentos son para las subclases: ExternalForce necesita sus
      // simProps en el init, el resto lo llama sin nada.
      init(..._args: unknown[]) {
    ...
    // Se pausa cuando el bloque no está a la vista: no tiene sentido simular
    // fluido para una sección que quedó tres pantallas más arriba.
  ...
  // Ajustes en caliente: si sólo cambió un número, no hace falta rearmar el
  // contexto WebGL (que igual lo haría el efecto de arriba).
  useEffect(() => {
