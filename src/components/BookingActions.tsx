...
export function BookingActions({ bookingId, status, viewer, proposal, finalPrice, paymentHandle, paymentHandleKind, paidPaymentId }: { bookingId: string; status: string; viewer: "cliente" | "profesional"; proposal?: Proposal | null; finalPrice?: number | null; paymentHandle?: string | null; paymentHandleKind?: string | null; paidPaymentId?: string | null }) {
  ...
  const paymentDetails = viewer === "cliente" && canRevealPaymentDetails(status) && paymentHandle ? <div className="select-all rounded-lg bg-white px-3 py-2 text-left text-sm"><p><span className="text-xs text-slate-400">{paymentHandleKind === "cvu" ? "CVU" : "Alias"}</span><br /><strong className={paymentHandleKind === "cvu" ? "font-mono" : ""}>{paymentHandle}</strong></p></div> : null;
  let content: React.ReactNode = null;
  if (viewer === "cliente" && status === "requested") content = activeProposal ? <div className="space-y-2 text-right"><p className="text-sm font-bold text-slate-900">Propuesta: {formatARS(activeProposal.amount)}</p><p className="text-xs text-slate-500">Vence {formatDateTime(activeProposal.expiresAt)}</p><div className="flex justify-end gap-2"><Button variant="cliente" disabled={busy} onClick={() => action("accept_proposal")} className="!py-1.5 !text-xs">Aceptar</Button><Button variant="outline" disabled={busy} onClick={() => action("reject_proposal")} className="!py-1.5 !text-xs">Rechazar</Button></div></div> : <Button variant="outline" disabled={busy} onClick={() => action("cancel")} className="!py-1.5 !text-xs">Cancelar acuerdo</Button>;
