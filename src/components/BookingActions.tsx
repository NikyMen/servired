...
export function BookingActions({ bookingId, status, viewer, proposal, finalPrice, paymentAlias, paymentCvu, paidPaymentId }: { bookingId: string; status: string; viewer: "cliente" | "profesional"; proposal?: Proposal | null; finalPrice?: number | null; paymentAlias?: string | null; paymentCvu?: string | null; paidPaymentId?: string | null }) {
  ...
  if (viewer === "cliente" && status === "requested") content = activeProposal ? <div className="space-y-2 text-right"><p className="text-sm font-bold text-slate-900">Propuesta: {formatARS(activeProposal.amount)}</p><p className="text-xs text-slate-500">Vence {formatDateTime(activeProposal.expiresAt)}</p><div className="flex justify-end gap-2"><Button variant="cliente" disabled={busy} onClick={() => action("accept_proposal")} className="!py-1.5 !text-xs">Aceptar</Button><Button variant="outline" disabled={busy} onClick={() => action("reject_proposal")} className="!py-1.5 !text-xs">Rechazar</Button></div></div> : <Button variant="outline" disabled={busy} onClick={() => action("cancel")} className="!py-1.5 !text-xs">Cancelar solicitud</Button>;
