...
export function AdminLocalidades({ rows }: { rows: Fila[] }) {
  return (
    <div className="space-y-4">
      <NuevaLocalidad />
      <div className="glass glass-solid overflow-hidden rounded-2xl">
...
function NuevaLocalidad() {
  ...
  return (
    <form key={JSON.stringify(state ?? "inicial")} action={formAction} className="glass glass-solid grid gap-3 rounded-2xl p-4 md:grid-cols-[1fr_1.4fr]">
      <div className="space-y-3">
        <h3 className="font-bold text-slate-900">Agregar localidad</h3>
        <label className="block text-xs font-semibold text-slate-600">Nombre
          <input name="name" defaultValue={values.name} required minLength={2} maxLength={60} placeholder="Goya" className="glass-field mt-1 w-full px-3 py-2 text-sm" />
        </label>
        <label className="block text-xs font-semibold text-slate-600">Provincia
          <input name="province" defaultValue={values.province} required minLength={2} maxLength={60} className="glass-field mt-1 w-full px-3 py-2 text-sm" />
        </label>
        ...
        <button disabled={pending} className="glass-btn px-4 py-2 text-sm disabled:opacity-60">{pending ? "Guardando…" : "Agregar"}</button>
      </div>
  ...
  return (
    <li className="p-3">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button type="button" onClick={() => setEditando((v) => !v)} className="text-xs font-semibold text-cliente hover:underline">{editando ? "Cerrar" : "Corregir punto"}</button>
        <form action={toggleLocalityAction}>
          <input type="hidden" name="id" value={row.id} />
          <input type="hidden" name="active" value={row.active ? "0" : "1"} />
          <button className={`glass-chip px-3 py-1.5 text-xs font-semibold ${row.active ? "text-slate-600" : "text-emerald-700"}`}>{row.active ? "Desactivar" : "Activar"}</button>
        </form>
      </div>
      {editando && (
        <form action={formAction} className="mt-3 space-y-2">
          ...
          <div className="flex items-center gap-3">
            <button disabled={pending} className="glass-btn px-4 py-2 text-sm disabled:opacity-60">{pending ? "Guardando…" : "Guardar punto"}</button>
