...
type Booking = { id: string; note: string | null; status: string; finalPrice: number | null; workSummary: string | null; paymentHandle: string | null; paymentHandleKind: string | null; proposals: Proposal[]; payments: Payment[] };
...
export function PaymentControls({ conversationId, viewer }: { conversationId: string; viewer: "cliente" | "profesional" }) {
  ...
  async function sendProposal() {
    ...
    const response = await fetch(`/api/conversaciones/${conversationId}/acuerdo`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "propose", amount: Number(amount), detail }) });
    const data = await response.json().catch(() => ({}));
    if (!response.ok) setError(data.error ?? "No pudimos enviar la propuesta."); else { setDetail(""); setAmount(""); await load(); }
    setBusy(false);
  }
  ...
  return <>
    ...
    {open && typeof document !== "undefined" && createPortal(<div className="fixed inset-0 z-[70] flex items-end justify-center bg-slate-950/30 p-3 sm:items-center" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <section role="dialog" aria-modal="true" aria-label="Trabajo, propuesta y pago" className="glass glass-solid animate-sheet-up max-h-[90dvh] w-full max-w-md overflow-y-auto rounded-3xl border-t-4 border-t-[var(--accent)] p-5 text-left shadow-2xl">
        ...
        {!booking && viewer === "profesional" && <div className="space-y-3"><input type="number" min="100" inputMode="numeric" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="Monto de la propuesta en ARS" className="glass-field px-3 py-2.5 text-sm" /><textarea rows={3} value={detail} onChange={(event) => setDetail(event.target.value)} placeholder="Detalle del trabajo (opcional)" className="glass-field resize-none px-3 py-2.5 text-sm" /><button type="button" disabled={busy || Number(amount) < 100} onClick={sendProposal} className="w-full rounded-xl bg-pro px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{busy ? "Enviando…" : "Enviar propuesta"}</button><p className="text-center text-[11px] text-slate-400">Solo podés tener una propuesta activa. El cliente debe rechazarla antes de que puedas enviar otra. Vence en 3 días.</p></div>}
        ...
        {booking && <div className="space-y-4"><div className="rounded-2xl bg-white/65 p-4"><div className="flex items-center justify-between gap-2"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Propuesta</p><StatusPill status={booking.status} /></div><p className="mt-2 whitespace-pre-wrap text-sm text-slate-700">{booking.note || "Servicio a convenir"}</p>{booking.workSummary && <p className="mt-3 rounded-xl bg-emerald-50 p-3 text-sm text-emerald-800">{booking.workSummary}</p>}</div><BookingActions bookingId={booking.id} status={booking.status} viewer={viewer} proposal={activeProposal} finalPrice={booking.finalPrice} paymentHandle={booking.paymentHandle} paymentHandleKind={booking.paymentHandleKind} paidPaymentId={paidPaymentId} /></div>}
