...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (isDuplicatePreinscriptionError(error)) return NextResponse.json({ error: "Ese correo ya está preinscripto. ¡Ya estás en la lista!" }, { status: 409 });
    console.error("No se pudo guardar la preinscripción", error);
