...
export default async function ProPanelPage({ searchParams }: { searchParams: Promise<{ tipo?: string; editarKyc?: string; mp?: string }> }) {
  ...
  if (!pro || pro.profileStatus === "changes_requested" || (pro.profileStatus === "approved" && editarKyc === "1")) {
    ...
    return <ProfessionalOnboardingForm categories={categories.map(({ id, name, icon, kind, parent }) => ({ id, name, icon, kind, parent }))} localities={localities.map(({ id, name, province }) => ({ id, name, province }))} initial={{
      ...
      legalName: existingKyc?.legalName, phone: existingKyc?.phone, birthDate: existingKyc?.birthDate.toISOString().slice(0, 10), cuil: existingKyc ? decryptKyc(existingKyc.cuilEncrypted) : undefined, address: existingKyc?.address, localityId: user.localityId, ofertasDependencia: cuenta?.ofertasDependencia ?? false,
    }} />;
  }
  ...
  const mpEstado = pro ? await estadoMercadoPago(pro.id) : null;
              ...
              return <li key={b.id}>
                <details className="glass glass-card group overflow-hidden rounded-2xl">
                  ...
                  <div className="space-y-4 border-t border-white/60 p-4">
                    ...
                    {b.payments.map((payment) => <p key={payment.id} className={`rounded-xl px-3 py-2 text-sm font-semibold ${payment.status === "pagado" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>Pago {payment.provider === "mercadopago" ? "por Mercado Pago" : "por alias"}: {formatARS(payment.amount)}{payment.commission > 0 && ` (recibís ${formatARS(payment.netAmount)} · comisión ServiRed ${formatARS(payment.commission)})`} · {payment.status}</p>)}
