...
export default async function MiPerfilProfesionalPage() {
  ...
  return <div className="mx-auto max-w-3xl space-y-6"><div><h1 className="text-2xl font-bold text-slate-900">Mi perfil</h1><p className="text-sm text-slate-500">Perfil de tipo {pro.providerType}. Cambiar el tipo o la identidad requiere una nueva verificación.</p>{activeJobs === 0 ? <a href="/pro?editarKyc=1" className="mt-2 inline-block text-sm font-semibold text-pro-dark hover:underline">Solicitar cambio de tipo o datos de identidad</a> : <p className="mt-2 text-xs text-amber-700">Terminá tus trabajos activos antes de solicitar ese cambio.</p>}</div><PerfilForm categories={categories} perfil={{ name: pro.name, avatarUrl: pro.avatarUrl, businessName: pro.businessName, headline: pro.headline, bio: pro.bio, address: pro.address, zone: pro.zone, categoryId: pro.categoryId, categoryIds: pro.categoryLinks.map((link) => link.categoryId), latitude: pro.latitude, longitude: pro.longitude, providerType: pro.providerType === "profesional" ? "profesional" : "oficio", paymentHandle: pro.paymentHandle, phone: pro.phone, yearsExperience: pro.yearsExperience }} /><TrabajosParticulares fotos={workSamples} /><Matriculas items={credenciales.map(({ createdAt: _creada, mimeType: _tipo, ...item }) => item)} rubros={rubros} /><AvisosCorreo inicial={await preferenciasAvisos(user.id)} /><BajaCuentaAviso /></div>;
}
